...
function Banzai:DisableTag(frame)
	frame.hasBTag = nil
end

function Banzai:UpdateTags(frame)
	if( not frame.fontStrings or not frame.hasBTag ) then return end
	
	for _, fontString in pairs(frame.fontStrings) do
		if( fontString.BANZAI ) then
...
ShadowUF.Tags.banzai:RegisterCallback(callback)

ShadowUF.Tags.customEvents["BANZAI"] = Banzai
...
Tags.defaultTags = {
	...
	["unit:situation"] = [[function(unit, unitOwner)
		local state = ShadowUF.Tags.banzai:GetUnitAggroByUnitId(unit)
		if( state ) then
	...
	["unit:color:aggro"] = [[function(unit, unitOwner)
		...
		return state and ShadowUF:Hex(1,0,0)
	end]],
...
Tags.defaultEvents = {
	...
	["unit:color:aggro"]	= "BANZAI",
	["unit:situation"]		= "BANZAI",
}
